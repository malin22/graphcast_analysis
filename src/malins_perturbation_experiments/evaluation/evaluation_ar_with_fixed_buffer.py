...
WEATHER_FEATURE = "AR"
THRESHOLD = 0.9
ACTIVATION_TYPE="raw_activations_single_perturbation" # "raw_activations_single_perturbation" or "raw_activation" or " "
...
def compute_ivt(ds):
    """
    Compute integrated vapor transport over 200--1000 hPa:

        IVT = sqrt[(1/g ∫ q*u dp)^2 + (1/g ∫ q*v dp)^2]
    """
    q = ds[Q_VAR]
    u = ds[U_VAR]
    v = ds[V_VAR]
    ...
    if "level" not in q.dims:
        raise ValueError(
            "Expected pressure-level variable with dimension 'level'."
        )

    # Restrict integration to 200--1000 hPa.
    ...
    # Pressure coordinate corresponding exactly to the retained fields.
    p = q["level"].values.astype(float)

    # Convert hPa to Pa for SI units.
    if np.nanmax(p) < 2000:
        p = p * 100.0

    # Integrate from low to high pressure.
    order = np.argsort(p)
    p_sorted = p[order]
