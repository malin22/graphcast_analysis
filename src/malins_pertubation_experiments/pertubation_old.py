...
for gamma in GAMMA:
    ...
    for center in centers:
        ...
        base_out_dir = (
            f"plots/malins_experiments/pertubation_experiments/"
            f"pertubation_threshold_{THRESHOLD}/"
            f"AR_gamma_{gamma}_{center_str}"
        )

        for t in pred.time.values:

            lead_hours = int(t / np.timedelta64(1, "h"))
            timestep_str = f"lead_{lead_hours:03d}h"

            
            timestep_dir = os.path.join(base_out_dir, f"timestep_{timestep_str}")
            os.makedirs(timestep_dir, exist_ok=True)

            out_path = os.path.join(timestep_dir, "prediction.nc")
            pred.sel(time=t).to_netcdf(out_path)

            print("Saved timestep prediction:", out_path)
