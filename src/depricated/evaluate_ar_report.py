...
Produces two core figures:
  ...
  Figure 2: early/late AR-mask dose responses plus the global mean ΔIVT
            trajectory relative to the unperturbed (gamma=0) forecast.
...
# -----------------------------------------------------------------------------
def compute_global_delta_trajectory(ivt_cache, lead_hours):
    """Area-weighted global mean ΔIVT relative to gamma=0 at every lead time."""
    control = ivt_cache[CONTROL_GAMMA]
    records = []
    for gamma in sorted(ivt_cache):
        for t_idx, lead_h in enumerate(lead_hours):
            delta = ivt_cache[gamma].isel(time=t_idx) - control.isel(time=t_idx)
            records.append({
                "gamma": float(gamma),
                "lead_hours": float(lead_h),
                "global_mean_delta_ivt": area_weighted_mean(delta),
            })
...
def draw_global_trajectory_panel(ax, global_metrics, title=None, show_legend=True):
    gammas = sorted(g for g in global_metrics["gamma"].unique() if not np.isclose(g, CONTROL_GAMMA))
    cmap = plt.get_cmap("coolwarm")
    max_abs_gamma = max(abs(g) for g in gammas)
...
def make_figure2(metrics, global_metrics):
    ...
    late_lead = float(available_leads[-1])

    dose_values = metrics[
        metrics["lead_hours"].isin([early_lead, late_lead])
    ][["delta_ivt_inside_mask_mean", "delta_ivt_outside_mask_mean"]].to_numpy()
    ymax = np.nanmax(np.abs(dose_values))
    margin = 0.08 * ymax if ymax > 0 else 1.0
    ylims = (-ymax - margin, ymax + margin)

    # Composite report figure.
    fig, axes = plt.subplots(
        1, 3, figsize=(12.4, 3.7),
        gridspec_kw={"width_ratios": [1, 1, 1.15]},
        constrained_layout=True,
    )
    draw_dose_panel(
        axes[0], metrics, early_lead, ylims,
        title=f"(a) Early forecast (+{format_lead_time(round(early_lead))})",
        show_legend=False,
    )
    draw_dose_panel(
        axes[1], metrics, late_lead, ylims,
        title=f"(b) Late forecast (+{format_lead_time(round(late_lead))})",
        show_legend=True,
    )
    axes[1].set_ylabel("")
    draw_global_trajectory_panel(
        axes[2], global_metrics,
        title="(c) Global perturbation response",
        show_legend=True,
    )

    stem = os.path.join(OUT_DIR, "figure2_ar_dose_response")
    fig.savefig(stem + ".png", bbox_inches="tight")
    plt.close(fig)
    print("Saved:", stem + ".png")

    # Standalone panels for LaTeX assembly.
    panel_fig, panel_ax = plt.subplots(figsize=(4.7, 3.5), constrained_layout=True)
    draw_dose_panel(
        panel_ax, metrics, early_lead, ylims,
        title=f"Early forecast (+{format_lead_time(round(early_lead))})",
        show_legend=True,
    )
    ...
    panel_fig, panel_ax = plt.subplots(figsize=(4.7, 3.5), constrained_layout=True)
    draw_dose_panel(
        panel_ax, metrics, late_lead, ylims,
        title=f"Late forecast (+{format_lead_time(round(late_lead))})",
        show_legend=True,
    )
    save_panel(panel_fig, "figure2b_late_dose_response.png")

    panel_fig, panel_ax = plt.subplots(figsize=(5.2, 3.5), constrained_layout=True)
    draw_global_trajectory_panel(
        panel_ax, global_metrics,
        title="Global perturbation response",
        show_legend=True,
    )
    save_panel(panel_fig, "figure2c_global_delta_ivt_trajectory.png")
...
def main():
    ...
    global_metrics = compute_global_delta_trajectory(ivt_cache, lead_hours)
    global_metrics_path = os.path.join(OUT_DIR, "global_delta_ivt_trajectory.csv")
