...
PCA_2019_2020_PATH = Path("/share/prj-4d/graphcast_shared/data/pca_components/512_PCs/layer8_only/pca_components_2019_2020_layer8.npy")

PCA_2021_PATH = Path("/share/prj-4d/graphcast_shared/data/pca_components/512_PCs/layer8_only/pca_components_2021_layer8.npy")

OUT_DIR = Path(
    "/home/student/s/sascholle/share/graphcast_analysis/plots/baselines/2019_2020_vs_2021_PC_validation"
)
...
def load_similarity_matrix(path: Path) -> np.ndarray:
    """Load the previously saved absolute cosine-similarity matrix."""
    if not path.exists():
        raise FileNotFoundError(f"Saved similarity matrix not found: {path}")

    sim = np.load(path)

    if sim.ndim != 2:
        raise ValueError(
            f"Expected a 2D similarity matrix, got shape {sim.shape}"
        )
...
def basis_overlap_summary(sim: np.ndarray) -> dict[str, float]:
    ...
    Note:
      ...
      of the overlap matrix. That is included too, but for a full 512-PC basis
...
def make_heatmap(sim: np.ndarray, out_path: Path) -> None:
    ...
    cbar=plt.colorbar(im, label="Absolute cosine similarity")
    cbar.ax.tick_params(labelsize=16,length=8,width=1.5)
    cbar.set_label( "Absolute cosine similarity", fontsize=20)
    plt.xlabel("2021 layer 8 PC", fontsize=23, labelpad=10)
    plt.ylabel("2019+2020 layer 8 PC", fontsize=23, labelpad=10)
    plt.tick_params(axis="both", which="major", labelsize=16, length=8, width=1.5)
    plt.title("2019+2020 vs 2021 PCA basis overlap, layer 8", fontsize=25, pad=20)
...
def main() -> None:
    ...
    # Load the matrix that was already calculated and saved.
    # abs_sim = load_similarity_matrix(NPY_PATH)

    X20 = load_components(PCA_2019_2020_PATH)
    ...
    n_pcs = min(X20.shape[0], X21.shape[0], 512)
    X20 = X20[:n_pcs]
    X21 = X21[:n_pcs]
    ...
    make_heatmap(abs_sim, heatmap_path)
