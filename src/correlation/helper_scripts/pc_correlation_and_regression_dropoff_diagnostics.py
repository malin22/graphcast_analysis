...
'''
python pc_correlation_and_regression_dropoff_diagnostics.py \
...
python pc_correlation_and_regression_dropoff_diagnostics.py \
  --input-json /home/student/s/sascholle/share/graphcast_analysis/plots/sabines_experiments/mapping_experiments/top_512_pcs/regression_pc_era5_mesh_m6_allvars_linear_results.json \
  --out-dir /home/student/s/sascholle/share/graphcast_analysis/plots/sabines_experiments/mapping_experiments/top_512_pcs/dropoff_diagnostics
...
def extract_regression_topk_rows(data, top_k=3, normalize_coefficients=False):
    rows = []
    ...
    for pc_name in sorted(data.keys(), key=pc_sort_key):
        ...
        if "coef_standardized" in pc_data and isinstance(pc_data["coef_standardized"], dict):
            feature_scores = {
                feature: abs(float(coef))
                for feature, coef in pc_data["coef_standardized"].items()
            }
        else:
            ranked = pc_data.get("ranked_features_standardized", [])
            feature_scores = {}
            for row in ranked:
                feature = row.get("feature", row.get("variable"))
                coef = row.get("abs_coefficient", row.get("coefficient"))
                if feature is not None and coef is not None:
                    feature_scores[feature] = abs(float(coef))

        if normalize_coefficients and feature_scores:
            denom = max(feature_scores.values())
            if denom > 0:
                feature_scores = {
                    feature: score / denom
        ...
        row = {
            "pc_name": pc_name,
            "pc_index": pc_idx,
            "val_r2": pc_data.get("val_r2"),
            "model_type": pc_data.get("model_type"),
            "alpha": pc_data.get("alpha"),
        }

        for k in range(top_k):
            if k < len(ranked_features):
                feature, score = ranked_features[k]
                row[f"top{k+1}_variable"] = feature
                row[f"top{k+1}_score"] = float(score)
            else:
...
def plot_regression_dropoff(rows, output_path, rolling_window=15):
    pc_indices = np.asarray([r["pc_index"] for r in rows], dtype=int)
    val_r2 = np.asarray([r["val_r2"] for r in rows], dtype=float)

    top1 = np.asarray([r["top1_score"] for r in rows], dtype=float)
    top2 = np.asarray([r["top2_score"] for r in rows], dtype=float)
    top3 = np.asarray([r["top3_score"] for r in rows], dtype=float)

    fig, axes = plt.subplots(nrows=2, ncols=1, figsize=(16, 9), sharex=True)

    axes[0].plot(pc_indices, val_r2, linewidth=1.2, label="Validation R2")
    axes[0].plot(
        pc_indices,
        rolling_mean(val_r2, rolling_window),
        color="black",
        linewidth=2.2,
    ...
    axes[0].set_title("Regression fit quality across PCA components")
    axes[0].grid(alpha=0.25)
    axes[0].legend()

    axes[1].plot(pc_indices, top1, linewidth=1.2, alpha=0.9, label="Top 1 coefficient")
    axes[1].plot(pc_indices, top2, linewidth=1.0, alpha=0.65, label="Top 2 coefficient")
    axes[1].plot(pc_indices, top3, linewidth=1.0, alpha=0.65, label="Top 3 coefficient")
    axes[1].plot(
        pc_indices,
        rolling_mean(top1, rolling_window),
        color="black",
    ...
    axes[1].set_ylabel("Abs standardized coefficient")
    axes[1].set_title("Strongest regression coefficients across PCA components")
...
def save_regression_rows_csv(rows, output_path, top_k=3):
    with open(output_path, "w") as f:
        columns = ["pc_name", "pc_index", "val_r2", "model_type", "alpha"]
        for k in range(top_k):
            columns.extend([
                f"top{k+1}_variable",
                f"top{k+1}_score",
...
def main():
    ...
    parser.add_argument(
        "--mode",
        choices=["correlation", "regression"],
        default="correlation",
    )
    parser.add_argument(
        "--normalize-regression-coefficients",
        action="store_true",
        help="Normalize top regression coefficients within each PC before plotting.",
    )
    ...
    if args.mode == "correlation":
        ...
        rows = extract_regression_topk_rows(
            data,
            top_k=args.top_k,
            normalize_coefficients=args.normalize_regression_coefficients,
        )

        save_regression_rows_csv(
            rows,
            args.out_dir / "pc_top_regression_features.csv",
            top_k=args.top_k,
        )

        plot_regression_dropoff(
            rows,
            args.out_dir / "pc_regression_dropoff_top3_and_r2_by_pc.png",
            rolling_window=args.rolling_window,
        )
